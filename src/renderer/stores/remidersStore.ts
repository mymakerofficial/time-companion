...
export const useRemindersStore = defineStore(
  'reminders',
  (): RemindersStore => {
    const storage = useLocalStorage<RemindersStorageSerialized>(
      'time-companion-reminders-store',
      { version: 0, reminders: [] },
    )
    ...
    function unsafeRemoveReminder(reminder: ReactiveCalendarReminder) {
      ...
      check(
        index !== -1,
        `Failed to remove reminder "${reminder.id}": Reminder does not exist in store.`,
      )
