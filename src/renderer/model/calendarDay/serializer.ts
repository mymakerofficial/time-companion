...
export function fromSerializedDay(
  serialized: SerializedCalendarDay,
  assets: DayDeserializationAssets,
): CalendarDayInit {
  return {
    id: serialized.id,
    date: parseDate(serialized.date),
    events: serialized.events.map((it) =>
      createEvent(fromSerializedEvent(it, assets)),
    ),
  }
}
