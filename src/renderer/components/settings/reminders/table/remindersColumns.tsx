    ...
    <span class='flex flex-row items-center gap-2'>
      ...
      {value.targetProject && (
        <>
          ...
          <span>{value.targetProject?.displayName}</span>
        </>
      )}
      {value.targetActivity && (
        <>
          <Slash class='size-3' />
          <span>{value.targetActivity?.displayName}</span>
        </>
      )}
    </span>
  )
...
function getActionsCell(
  ...
  return (
    <Button
      onClick={() => options.onOpenEditReminderDialog(row.original.id)}
      variant='ghost'
      size='icon'
    >
      <Pencil class='size-4' />
...
export function createRemindersColumns(options: RemindersColumnsOptions) {
  const { t } = useI18n()

  return [
    columnHelper.accessor('name', {
      header: ({ column }) =>
        getSortableHeader(column, t('settings.reminders.table.columns.name')),
    }),
    columnHelper.accessor('startAt', {
      header: ({ column }) =>
        getSortableHeader(
          column,
          t('settings.reminders.table.columns.startAt'),
        ),
