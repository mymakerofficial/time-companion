...
const containerPosition = computed(() => {
  ...
  const startRow =
    durationToGridRows(durationSinceStartOfDay(startAt)) + startOffset
  const spanRows = durationToGridRows(
    durationBetween(startAt, endAt ?? now.value),
  )
      ...
      >
        <div class="text-sm flex items-center gap-2 text-nowrap">
          <div class="font-medium flex items-center gap-1">
            ...
            <Slash
              v-if="isNotEmpty(event.activityDisplayName)"
              class="size-3"
            />
            <span v-if="isNotEmpty(event.activityDisplayName)">{{
